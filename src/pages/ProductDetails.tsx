...
export const Details = () => {
  ...
  const {
    data:prodcutlist , 
    isPending:loadingProductDetails
  } = useGetProductDeatils(id?? "" ); //send id to useGetProductdetails page and stores the productDetails sent from it in a constant name data
 
  const data = prodcutlist
   console.log(data)
  ...
  const setBuyNowProduct = useProduct((state) => state.setBuyNowProduct);
    ...
    let cartData:CartData = cartItems ? JSON.parse(cartItems) : {};
    ...
    if (itemIndex !== -1) {
      ...
      userCart.push({ ...data, quantity: 1,token: token ?? "",    });
    }
 ...
 return (
<div className="flex flex-col md:gap-10 min-h-screen bg-white text-black p-6">
  
  {/* Title */}
  <div className="flex justify-center  items-center ">
  <h1 className="text-xl md:text-3xl font-extrabold text-gray-900 border-b-4 border-amber-500  p-2">
    {data.title}
  </h1>
 
</div>
  <hr className="my-4 " />
  <div className=" hidden md:grid  gap-8  grid-cols-2 p-4 bg-gray-50 rounded-lg shadow-sm">
    ...
    <div className="flex flex-col gap-5 p-6 bg-white rounded-lg shadow-md">
      ...
      <div className=" hidden lg:flex gap-3">
        ...
        <button className="flex items-center gap-2 border border-amber-500 text-amber-500 p-2 rounded-md shadow hover:bg-amber-50">
          <FaHeart />
          Favourite
        </button>
      </div>
  ...
  <div className="  md:hidden text-sm  gap-2  bg-gray-50 rounded-lg shadow-sm">
    ...
    <div className="flex flex-col items-center gap-4 ">
      {data.images && data.images.length > 0 && (
        <>
          <img
            className=" w-2/4  " src={data.images[selectedImageIndex]} alt={data.title} />
    ...
    <div className="flex  flex-col gap-5 p-6 bg-white rounded-lg shadow-md">
      ...
      <div>
        <h1 className="font-semibold  text-gray-800">Price:</h1>
        <span className="flex gap-5 items-center mt-2">
      ...
      <div>
        <h1 className="font-semibold  text-gray-800">Brand:</h1>
        <p>{data.brand}</p>
      </div>
      <div>
        <h1 className="font-semibold  text-gray-800">Product Description:</h1>
        <p>{data.description}</p>
      </div>
      <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-md">
        <div>
          <h1 className="font-semibold  text-gray-800">Shipping:</h1>
          <p>{data?.shippingInformation}</p>
        </div>
        <div>
          <h1 className="font-semibold  text-gray-800">Return Policy:</h1>
          <p>{data?.returnPolicy}</p>
        </div>
        <div>
          <h1 className="font-semibold text-gray-800">Availability:</h1>
          <p>{data?.availabilityStatus}</p>
        </div>
        <div>
          <h1 className="font-semibold  text-gray-800">Warranty:</h1>
          <p>{data?.warrantyInformation}</p>
        </div>
      </div>
      <div className=" hidden lg:flex gap-3">
  ...
  {/* Ratings */}
 <h1 className="font-semibold  md:text-2xl text-gray-900 mb-2">Rating & Reviews</h1>
...
<div className="flex gap-8 p-6 bg-white rounded-lg shadow-md">
  ...
  <div className="grid gap-4 grid-cols-1 md:grid-cols-2 flex-1">
    {data.reviews?.map((review, index) => (
      <div
        key={index}
        className="p-4 text-sm bg-gray-50 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition"
      >
        <h3 className="font-semibold md:text-lg text-gray-800 flex justify-between items-center">
          {review.reviewerName}
          <span className=" hidden md:block text-xs text-gray-500">{new Date(review.date).toLocaleDateString()}</span>
        </h3>
